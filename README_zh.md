...
GRAVITY 是一个将 RNA velocity 推断与动态基因调控网络建模结合起来的深度学习框架。它整合未剪接/已剪接 RNA 丰度、细胞嵌入和先验基因调控网络，通过调控网络感知的 attention 结构联合建模细胞状态转移、基因特异性转录动力学，以及动态调控网络重连。
...
胰腺内分泌发生参考权重位于 `data/pancreas/reference_checkpoints/`。对应的参考导出
